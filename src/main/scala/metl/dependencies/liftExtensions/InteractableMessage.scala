...
class ClientMessageBroker(messageTemplate: NodeSeq,
                          messageSelector: String,
                          labelSelector: String,
                          contentSelector: String,
                          closeSelector: String,
                          onMessageArrival: (ClientMessage) => Unit,
                          onMessageRemoval: (ClientMessage) => Unit) {
  private var visibleMessages = List.empty[ClientMessage]
  private def removeMessageFromVisible(cm: ClientMessage) =
    visibleMessages = visibleMessages.filterNot(m => m == cm)
  ...
  def processMessage(cm: ClientMessage): Unit = {
    cm match {
      ...
      case i: InteractableMessage => {
        val message = InteractableMessage(
          i.scope,
          i.role,
          i.title,
          (cmi) => {
...
abstract class ClientMessage(id: String,
                             incomingRole: Box[String] = Empty,
                             incomingTitle: Box[String] = Empty,
                             removalFunc: (ClientMessage) => Unit,
                             template: NodeSeq,
                             messageSelector: String,
                             labelSelector: String,
                             contentSelector: String,
  ...
  def renderMessage: NodeSeq = {
    ...
    ((labelSelector + " *") #> title.openOr("Response") &
      ...
      closeSelector #> ((n: NodeSeq) =>
        if (cancellable) a(() => done, n) else NodeSeq.Empty)).apply(template)
  }
...
case object Clear
    extends ClientMessage("clearSingleton",
                          Empty,
                          Empty,
                          (cm) => {},
                          NodeSeq.Empty,
...
object InteractableMessage {
  def apply(scope: InteractableMessage => NodeSeq,
            role: Box[String] = Empty,
            incomingTitle: Box[String] = Empty,
            removalFunc: (ClientMessage) => Unit = (cm) => {},
            cancellable: Boolean = true,
            template: NodeSeq = NodeSeq.Empty,
            messageSelector: String = "",
            labelSelector: String = "",
            contentSelector: String = "",
            closeSelector: String = "",
            id: String = nextFuncName) =
    new InteractableMessage(scope,
                            role,
                            incomingTitle,
                            removalFunc,
                            cancellable,
                            ...
                            closeSelector,
                            id)
  def unapply(in: InteractableMessage): Option[
    Tuple11[InteractableMessage => NodeSeq,
            Box[String],
            Box[String],
            ClientMessage => Unit,
            Boolean,
...
class InteractableMessage(val scope: InteractableMessage => NodeSeq,
                          override val role: Box[String] = Empty,
                          val incomingTitle: Box[String] = Empty,
                          val removalFunc: (ClientMessage) => Unit = (cm) => {},
  ...
  override def equals(a: Any) = a match {
    case InteractableMessage(aScope,
                             aRole,
                             aIncomingTitle,
                             aRemovalFunc,
                             aCancellable,
                             ...
                             aCloseSelector,
                             aId) =>
      aScope == scope && aRole == role && aIncomingTitle == incomingTitle && aRemovalFunc == removalFunc && aCancellable == cancellable && aTemplate == template && aMessageSelector == messageSelector && aLabelSelector == labelSelector && aContentSelector == contentSelector && aCloseSelector == closeSelector && aId == id
    case _ => false
  }
}

case class SimpleTextAreaInteractableMessage(
    messageTitle: String,
    body: String,
    defaultValue: String,
    onChanged: (String) => Boolean,
    customError: Box[() => Unit] = Empty,
    override val role: Box[String] = Empty)
    extends InteractableMessage(
      (i) => {
...
case class SimpleMultipleButtonInteractableMessage(
    messageTitle: String,
    body: String,
...
case class SimpleRadioButtonInteractableMessage(
    messageTitle: String,
    body: String,
    radioOptions: Map[String, () => Boolean],
    defaultOption: Box[String] = Empty,
    customError: Box[() => Unit] = Empty,
    override val role: Box[String] = Empty)
    extends InteractableMessage(
      (i) => {
...
case class SimpleDropdownInteractableMessage(
    messageTitle: String,
    body: String,
    dropdownOptions: Map[String, () => Boolean],
    defaultOption: Box[String] = Empty,
    customError: Box[() => Unit] = Empty,
    override val role: Box[String] = Empty)
    extends InteractableMessage(
...
case class SpamMessage(content: NodeSeq,
                       override val role: Box[String] = Empty,
                       incomingTitle: Box[String] = Empty,
                       removalFunc: (ClientMessage) => Unit = (cm) => {},
                       override val cancellable: Boolean = true,
