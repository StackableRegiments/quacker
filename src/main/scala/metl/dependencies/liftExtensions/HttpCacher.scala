...
class HttpCacher {
  ...
  private def makeCacheHeaders(binary: CachedBinary, expiry: Long) = {
    ...
    List(
      ...
      "Cache-Control" -> "max-age=%d, must-revalidate".format(
        if (maxAge < 0) 0 else maxAge),
      "ETag" -> binary.checksum
    )
  }

  def constructResponse(binary: CachedBinary,
                        contentType: String,
                        expiry: TimeSpan) = {
    requestEtag match {
      case Full(etag) if (etag == binary.checksum) =>
        InMemoryResponse(Array.empty[Byte], List(), Nil, 304)
      case _ =>
        InMemoryResponse(
          binary.data,
          List("Content-Type" -> contentType) ::: makeCacheHeaders(binary,
                                                                   expiry),
          Nil,
          200)
    }
