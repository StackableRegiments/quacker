...
class FormAuthenticationSystem(mod: FormAuthenticator)
    extends LiftAuthenticationSystem {
  override def dispatchTableItemFilter = (r) => false
  protected def dispatchTableItemFilterInternal: Req => Boolean =
    (r) => !mod.checkWhetherAlreadyLoggedIn
  override def dispatchTableItem(r: Req) = Full(mod.constructResponse(r))
  LiftRules.dispatch.prepend {
    case r @ Req("formLogon" :: Nil, _, _)
        if dispatchTableItemFilterInternal(r) && r.post_? =>
      () =>
        {
          ...
          mod.tryLogin(username, password) match {
            ...
            case Right(false) => {
              Full(
                mod.constructResponseWithMessages(
                  r,
                  List(
...
class FormAuthenticator(
    ...
    verifyCredentials: Tuple2[String, String] => LiftAuthStateData,
  ...
  override def checkWhetherAlreadyLoggedIn: Boolean =
    Stopwatch.time("FormAuthenticator.checkWhetherAlreadyLoggedIn",
                   alreadyLoggedIn() || InSessionLiftAuthState.is.authenticated)

  def tryLogin(username: Box[String],
               password: Box[String]): Either[Throwable, Boolean] = {
    val response = for (u <- username;
                        p <- password) yield {
      try {
  ...
  protected def makeUrlFromReq(req: Req): String = {
    "%s%s".format(
      req.uri,
      req.request.queryString.map(qs => "?%s".format(qs)).openOr(""))
  }
  override def constructResponse(req: Req) =
    constructResponseWithMessages(req, List.empty[String])
  def constructResponseWithMessages(req: Req,
                                    additionalMessages: List[String] =
                                      List.empty[String]) =
    Stopwatch.time(
      "FormAuthenticator.constructReq", {
        val loginPageNode = (
          "%s [method]".format(formSelector) #> "POST" &
            "%s [action]".format(formSelector) #> "/formLogon" &
            "%s *".format(formSelector) #> {
              (formNode: NodeSeq) =>
                {
        ...
        LiftRules.convertResponse(
          (
            ...
            S.getResponseHeaders(
              LiftRules.defaultHeaders((loginPageNode, req))),
            S.responseCookies,
            req
          ))
      }
