...
class CASAuthenticationSystem(mod: CASAuthenticator)
    extends LiftAuthenticationSystem {
  override def dispatchTableItemFilter =
    (r) =>
      ((!mod.checkWhetherAlreadyLoggedIn) && (!mod.checkReqForCASCookies(r)))
  override def dispatchTableItem(r: Req) = Full(mod.constructResponse(r))
}

class CASAuthenticator(realm: String,
                       baseUrl: String,
                       httpClient: Option[IMeTLHttpClient],
                       alreadyLoggedIn: () => Boolean,
                       onSuccess: (LiftAuthStateData) => Unit)
    extends LiftAuthenticator(alreadyLoggedIn, onSuccess) {
  ...
  private def getHttpClient: IMeTLHttpClient =
    httpClient.getOrElse(Http.getClient)

  override def checkWhetherAlreadyLoggedIn: Boolean =
    Stopwatch.time("CASAuthenticator.checkWhetherAlreadyLoggedIn",
                   alreadyLoggedIn() || InSessionLiftAuthState.is.authenticated)

  def checkReqForCASCookies(req: Req): Boolean =
    Stopwatch.time(
      "CASAuthenticator.checkReqForCASCookies", {
  ...
  private def ticketlessUrl(originalRequest: Req): String =
    Stopwatch.time(
      "CASAuthenticator.ticketlessUrl", {
        ...
        val newParams = originalRequest.params.toList
          .sortBy(_._1)
          .foldLeft("")((acc, param) =>
            param match {
              case Tuple2(paramName, listOfParams)
                  if (paramName.toLowerCase == "ticket") =>
                acc
              case Tuple2(paramName, listOfParams) => {
                val newItem = "%s=%s".format(
                  URLEncoder.encode(paramName, "utf-8"),
                  URLEncoder.encode(listOfParams.mkString(""), "utf-8"))
                acc match {
                  case "" => acc + "?" + newItem
                  case _  => acc + "&" + newItem
                }
  ...
  private def verifyCASTicket(req: Req): LiftAuthStateData =
    Stopwatch.time(
      "CASAuthenticator.verifyCASTicket", {
        req.param("ticket") match {
          case Full(ticket) => {
            val verifyUrl = baseCasUrl + serviceValidatePath + "?ticket=%s&service=%s"
            val casValidityResponse = getHttpClient.getAsString(
              verifyUrl.format(ticket,
                               URLEncoder.encode(ticketlessUrl(req), "utf-8")))
            val casValidityResponseXml = xml.XML.loadString(casValidityResponse)
            val state =
              for (success <- (casValidityResponseXml \\ "authenticationSuccess");
                   user <- (success \\ "user"))
                yield
                  LiftAuthStateData(true,
                                    user.text,
  ...
  override def constructResponse(req: Req) =
    Stopwatch.time("CASAuthenticator.constructReq", {
      val url =
        redirectUrl.format(URLEncoder.encode(ticketlessUrl(req), "utf-8"))
