...
trait Synched {
  ...
  def syncReadUnlessPredicateThenWrite[A](pred: () => Boolean,
                                          fr: () => A,
                                          fw: () => A): A = {
    rl.lock
    val ret = if (pred()) {
...
class SynchronizedWriteMap[A, B](
    collection: scala.collection.mutable.HashMap[A, B] =
      scala.collection.mutable.HashMap.empty[A, B],
    updateOnDefault: Boolean = false,
    defaultFunction: Function[A, B] = (k: A) => null.asInstanceOf[B])
    extends Synched {
  private var coll = collection
  private var defaultFunc: Function[A, B] = defaultFunction
  def +=(kv: (A, B)): SynchronizedWriteMap[A, B] =
    syncWrite[SynchronizedWriteMap[A, B]](() => {
      coll.+=(kv)
      this
    })
  def -=(k: A): SynchronizedWriteMap[A, B] =
    syncWrite[SynchronizedWriteMap[A, B]](() => {
      coll.-=(k)
      this
    })
  ...
  def updated(k: A, v: B): SynchronizedWriteMap[A, B] =
    syncWrite[SynchronizedWriteMap[A, B]](() => {
  ...
  def transform(f: (A, B) => B): SynchronizedWriteMap[A, B] =
    syncWrite[SynchronizedWriteMap[A, B]](() => {
      coll.transform(f)
      this
    })
  def retain(p: (A, B) => Boolean): SynchronizedWriteMap[A, B] =
    syncWrite[SynchronizedWriteMap[A, B]](() => {
      coll.retain(p)
      this
    })
  ...
  def values: scala.collection.Iterable[B] =
    syncRead(() => coll.map(kv => kv._2))
  def valuesIterator: Iterator[B] = syncRead(() => coll.valuesIterator)
  def foreach[U](f: ((A, B)) => U) = syncRead(() => coll.foreach(f))
  def apply(k: A): B =
    syncReadUnlessPredicateThenWrite(() => isDefinedAt(k),
                                     () => coll.apply(k),
                                     () => default(k))
  def withDefault(f: (A) => B): SynchronizedWriteMap[A, B] =
    syncWrite(() => {
      this.defaultFunc = f
      this
    })
  ...
  def map(f: ((A, B)) => Any): scala.collection.mutable.Iterable[Any] =
    syncRead(() => coll.map(f))
  override def clone: SynchronizedWriteMap[A, B] =
    syncRead(
      () =>
        new SynchronizedWriteMap[A, B](coll.clone,
                                       updateOnDefault,
                                       defaultFunction))
  override def toString =
    "SynchronizedWriteMap(" + coll
      .map(kv => "(%s -> %s)".format(kv._1, kv._2))
      .mkString(", ") + ")"
  override def equals(other: Any) =
    (other.isInstanceOf[SynchronizedWriteMap[A, B]] && other
      .asInstanceOf[SynchronizedWriteMap[A, B]]
      .toList == this.toList)
}
