...
object CryptoTester extends Logger {
  val seeds =
    List("hello all", "8charact", "16characterslong", "a worm", "15", "a")
  ...
  def testCommutative(c1: Crypto, c2: Crypto, input: String): Boolean = {
    try {
      ...
      List(s1, s2, s3, s4, s5, s6, s7, s8).foreach(li =>
        trace("step: %s".format(li.toString)))
...
class DESCBCCrypto
    extends Crypto("DESede", "CBC", "ISO10126Padding", None, None)
class DESECBCrypto
    extends Crypto("DESede", "ECB", "ISO10126Padding", None, None)

//this is working AES
class AESCBCCrypto extends Crypto("AES", "CBC", "ISO10126Padding", None, None)

class Crypto(cipherName: String = "AES",
             cipherMode: String = "CBC",
             padding: String = "ISO10126Padding",
             inputKey: Option[Array[Byte]] = None,
             inputIv: Option[Array[Byte]] = None) {
  ...
  val cipher = List(cipherName, cipherMode, padding)
    .filter(i => i.length > 0)
    .mkString("/")
  val (ecipher, dcipher) = Stopwatch.time(
    "Crypto(%s,%s,%s)".format(cipher, inputKey, inputIv), {
          ...
          shouldUseInput match {
            case true => {
              val p = inputKey
                ...
                .getOrElse(BigInteger.probablePrime(512, new SecureRandom))
              val q = inputIv
                .map(i => new BigInteger(i))
                .getOrElse(BigInteger.probablePrime(512, new SecureRandom))
              val n = p.multiply(q)
              ...
              val modulus = n
              var pubKeyWorking = java.lang.Math.abs(
                (modulus
                  .divide(new BigInteger("100"))
                  .intValue * new java.util.Random().nextFloat).toInt)
              var pubKeyFound = false
              while (pubKeyFound == false) {
                print(pubKeyWorking + ",")
                if (phi
                      .gcd(new BigInteger(pubKeyWorking.toString))
        ...
        case _ => {
          ...
          val internalKey = cipherName match {
            case "DES" => {
              kgen.init(56)
              inputKey
                .map(i => new SecretKeySpec(i, cipherName))
                .getOrElse(kgen.generateKey)
            }
            case "AES" => {
              kgen.init(128)
              inputKey
          ...
          val internalIv = cipherName match {
            ...
            case "DESede" =>
              new IvParameterSpec(
                inputIv.getOrElse(Array[Byte](1, 2, 3, 4, 5, 6, 7, 8)))
            case _ =>
              new IvParameterSpec(
                inputIv.getOrElse(kgen.generateKey.getEncoded))
          }
  ...
  def cloneCrypto =
    Stopwatch.time(
      "Crypto.cloneCrypto",
      new Crypto(cipherName, cipherMode, padding, inputKey, inputIv))
