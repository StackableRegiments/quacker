...
case class RedirectException(
    message: String,
    exceptions: List[Throwable] = List.empty[Throwable])
    extends Exception(message) {}
case class RetryException(message: String,
                          exceptions: List[Throwable] = List.empty[Throwable])
    extends Exception(message) {}
case class WebException(message: String, code: Int, path: String)
    extends Exception(message) {}
...
trait IMeTLHttpClient {
  ...
  def getExpectingHTTPResponse(
      ...
      exceptions: List[Throwable] = List.empty[Throwable],
      startTime: Long = new Date().getTime): HTTPResponse

  def getAsString(uri: String): String =
    getAsString(uri, List.empty[(String, String)])
  def getAsString(uri: String,
                  additionalHeaders: List[(String, String)]): String

  def getAsBytes(uri: String): Array[Byte] =
    getAsBytes(uri, List.empty[(String, String)])
  def getAsBytes(uri: String,
                 additionalHeaders: List[(String, String)]): Array[Byte]

  def postBytes(uri: String, bytes: Array[Byte]): Array[Byte] =
    postBytes(uri, bytes, List.empty[(String, String)])
  ...
  def postBytesExpectingHTTPResponse(
      uri: String,
      bytes: Array[Byte],
      additionalHeaders: List[(String, String)]): HTTPResponse

  def postForm(uri: String, postItemList: List[(String, String)]): Array[Byte] =
    postForm(uri, postItemList, List.empty[(String, String)])
  ...
  def postUnencodedForm(uri: String,
                        postItemList: List[(String, String)]): Array[Byte] =
    postUnencodedForm(uri, postItemList, List.empty[(String, String)])
  def postUnencodedForm(uri: String,
                        postItemList: List[(String, String)],
                        additionalHeaders: List[(String, String)]): Array[Byte]
...
case class HTTPResponse(
    requestUrl: String,
    actOnConn: (ManagedClientConnection, String, String) => Unit,
    bytes: Array[Byte],
    statusCode: Int,
...
class CleanHttpClient(connMgr: ClientConnectionManager)
    extends DefaultHttpClient(connMgr)
    with IMeTLHttpClient
    with Logger {
  ...
  protected def addAdditionalHeaders(
      message: AbstractHttpMessage,
      additionalHeaders: List[(String, String)]): Unit = {
    if (additionalHeaders.length > 0)
      additionalHeaders.foreach(header =>
        message.addHeader(new BasicHeader(header._1, header._2)))
  }
  ...
  protected def applyDefaultHeaders(message: AbstractHttpMessage,
                                    uri: URI): Unit = {
  ...
  protected def applyAuthHeader(message: AbstractHttpMessage,
                                uri: URI): Unit = {
    val host = uri.toString
    val constructedHost = determineScheme(uri) + "://" + determineHost(uri) + ":" + determinePort(
      uri) + "/" + determinePath(uri)
    authorizations.map(auth =>
      auth match {
        case (domain, ("anonymous", "unauthorized")) => {}
        case (domain, (username, password))
            if (host.contains(domain) || constructedHost
              .contains(domain) || domain == "" || domain == "*") => {
          val authorizationString = SecurityHelpers.base64Encode(
            ("%s:%s".format(username, password)).getBytes("UTF-8"))
          message.addHeader(
            new BasicHeader("Authorization",
                            "Basic %s".format(authorizationString)))
        }
    })
  }
  protected def applyHostHeader(message: AbstractHttpMessage,
                                uri: URI): Unit = {
    ...
    Stopwatch.time(
      "Http.withConn", {
        try {
          ...
          val conn =
            connRequest.getConnection(connectionTimeout, TimeUnit.SECONDS)
          try {
            ...
            if (conn.isResponseAvailable(readTimeout)) {
              ...
              val responseOutput = HTTPResponse(
                uri,
                actOnConn,
                tempOutput,
                response.getStatusLine.getStatusCode,
                Map(response.getAllHeaders.toList.map(h =>
                  (h.getName, h.getValue)): _*),
                start,
                end,
                retryNumber,
                redirectNumber,
              ...
              connMgr.releaseConnection(conn,
                                        ...
                                        TimeUnit.SECONDS)
              throw new Exception(
                "Socket timeout - No data from: %s".format(uri))
            }
          } catch {
            case ex: Throwable => {
              conn.abortConnection
              executeHttpConnAction(uri,
                                    actOnConn,
                                    retryNumber + 1,
                                    redirectNumber,
                                    exceptionsSoFar ::: List(ex),
                                    start)
            }
          }
        } catch {
  ...
  protected def displayMetrics(conn: ManagedClientConnection): Unit = {
    ...
    debug(
      "rec'd:     %s (%s bytes)".format(m.getResponseCount,
                                        m.getReceivedBytesCount))
  }
  override def postBytes(uri: String,
                         bytes: Array[Byte],
                         additionalHeaders: List[(String, String)] =
                           List.empty[(String, String)]): Array[Byte] =
    Stopwatch.time("Http.postBytes", {
      respondToResponse(
        postBytesExpectingHTTPResponse(uri, bytes, additionalHeaders),
        additionalHeaders).bytes
    })
  override def postBytesExpectingHTTPResponse(
    ...
    : HTTPResponse =
    Stopwatch.time(
      "Http.postBytesExpectingHTTPResponse", {
        val bytePostingPost =
          (conn: ManagedClientConnection, url: String, path: String) => {
            val correctlyFormedUrl = new URI(url)
            val postMethod = new BasicHttpEntityEnclosingRequest("POST", path) {
              override val expectContinue = false
            }
            ...
            postMethod.addHeader(
              new BasicHeader("Content-Length",
                              postEntity.getContentLength.toString))
  ...
  override def postForm(uri: String,
                        ...
                        additionalHeaders: List[(String, String)] =
                          List.empty[(String, String)]): Array[Byte] =
    Stopwatch.time("Http.postForm", {
      respondToResponse(
        postFormExpectingHTTPResponse(uri, postItemList, additionalHeaders),
        additionalHeaders).bytes
    })
  override def postFormExpectingHTTPResponse(
    ...
    : HTTPResponse =
    Stopwatch.time(
      "Http.postFormExpectingHTTPResponse", {
        val formPostingPost =
          (conn: ManagedClientConnection, url: String, path: String) => {
            val correctlyFormedUrl = new URI(url)
            val postMethod = new BasicHttpEntityEnclosingRequest("POST", path) {
  ...
  override def postUnencodedForm(uri: String,
                                 ...
                                 additionalHeaders: List[(String, String)] =
                                   List.empty[(String, String)]): Array[Byte] =
    Stopwatch.time("Http.postUnencodedForm", {
      respondToResponse(
        postUnencodedFormExpectingHttpResponse(uri,
                                               postItemList,
                                               additionalHeaders),
        additionalHeaders).bytes
    })
  override def postUnencodedFormExpectingHttpResponse(
    ...
    : HTTPResponse =
    Stopwatch.time(
      "Http.postUnencodedFormExpectingHTTPResponse", {
        val unencodedFormPostingPost =
          (conn: ManagedClientConnection, url: String, path: String) => {
            val correctlyFormedUrl = new URI(url)
            val postMethod = new BasicHttpEntityEnclosingRequest("POST", path) {
              override val expectContinue = false
  ...
  override def getAsBytes(uri: String,
                          additionalHeaders: List[(String, String)] =
                            List.empty[(String, String)]): Array[Byte] =
    Stopwatch.time("Http.getAsBytes", {
      respondToResponse(getExpectingHTTPResponse(uri, additionalHeaders),
                        additionalHeaders).bytes
    })
  ...
  protected def executeHttpConnAction(
      uri: String,
      actOnConn: (ManagedClientConnection, String, String) => Unit,
      retriesSoFar: Int = 0,
      redirectsSoFar: Int = 0,
  ...
  def respondToResponse(response: HTTPResponse,
                        additionalHeaders: List[(String, String)] =
                          List.empty[(String, String)]): HTTPResponse = {
    ...
    val cookieString = cookies
      .map(cookieList => {
        ...
        cookiesContained.map(cookie => {
          ...
          val cookieDomain =
            cookieParams.find(p => p.getName.toLowerCase == "domain") match {
  ...
  protected def storeClientCookies(newHeaders: List[Header]): Unit = {
    val newCookies = newHeaders.map(header =>
      header.getElements.toList.map(item => (item.getName, header)))
    newCookies.foreach(newCookieList =>
      newCookieList.foreach(newCookie =>
        cookies = cookies.updated(newCookie._1, newCookie._2)))
  }
}
...
object Http {
  ...
  private val getSchemeRegistry = {
    ...
    val sslSf = new org.apache.http.conn.ssl.SSLSocketFactory(
      ssl_ctx,
  ...
  def getClient(headers: List[(String, String)]): CleanHttpClient =
    Stopwatch.time(
      "Http.getClient(headers)", {
        val newHeaders =
          headers.map(tup => new BasicHeader(tup._1, tup._2)).toList
        val client = new CleanHttpClient(getConnectionManager) {
