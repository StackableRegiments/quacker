...
case object Refresh
class PeriodicallyRefreshingVar[T](acceptedStaleTime: TimeSpan,
                                   valueCreationFunc: () => T)
    extends LiftActor {
  private var lastResult: T = valueCreationFunc()
  scheduleRecheck
  private def scheduleRecheck: Unit =
...
class ChangeNotifyingSessionVar[T](dflt: => T) extends SessionVar[T](dflt) {
  ...
  def unsubscribe(handler: T => Unit) =
    onChange = onChange.filterNot(_ == handler)
}
/*
