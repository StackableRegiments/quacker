...
object BugReport {
  ...
  private val successTemplate =
    Templates(List("_bugReportSuccess")).openOr(NodeSeq.Empty)
  private val failureTemplate =
    Templates(List("_bugReportFailure")).openOr(NodeSeq.Empty)
  ...
  private def successResponse(message: String) =
    XhtmlResponse(
      constructNode(("#message" #> Text(message)).apply(successTemplate)),
      Full(DocType.xhtmlTransitional),
      List.empty[Tuple2[String, String]],
      List.empty[HTTPCookie],
      200,
      true
    )
  private def failResponse(message: String) =
    XhtmlResponse(
      constructNode(("#message" #> Text(message)).apply(failureTemplate)),
      Full(DocType.xhtmlTransitional),
      List.empty[Tuple2[String, String]],
      List.empty[HTTPCookie],
      500,
  ...
  private def emailBody(user: String,
                        email: String,
                        name: String,
                        time: String,
                        location: String,
  ...
  private def getParam(incomingRequest: Req,
                       paramName: String,
                       default: String) = {
  ...
  def handleBug(req: Req): LiftResponse = {
    tryo({
      ...
      if (name != unknownName || description != unknownDescription) {
        val message =
          emailBody(reportingUser, email, name, time, location, description)
        Mailer.sendMail(From("bug.reporter@stackableregiments.com"),
                        Subject("bug report"),
                        PlainMailBodyType(message) :: List(
                          To("bug.acceptort@stackableregiments.com")): _*)
