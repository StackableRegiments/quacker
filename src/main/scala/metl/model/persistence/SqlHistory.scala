...
class NamedConnectionIdentifier(override val jndiName: String) extends ConnectionIdentifier
...
object DBVendors {
  def getVendor(driverName: String,
    ...
    new ApacheDbcpStandardDBVendor(driverName, dbUrl, dbUser, dbPassword, dbSettings)
  }
}
...
case class DBSettings(
    ...
    allowTemporaryPoolExpansion: Boolean = DBSettingsDefaults.allowTemporaryPoolExpansion,
    maxPoolSize: Int = DBSettingsDefaults.maxPoolSize,
    doNotExpandBeyond: Int = 50,
    maxAttemptsToGetAConnection: Option[Int] = DBSettingsDefaults.maxAttemptsToGetAConnection,
    ...
    waitTimeIntervalForConnection: Long = DBSettingsDefaults.waitTimeIntervalForConnection,
...
class ApacheDbcpStandardDBVendor(
  ...
  override def newConnection(name: ConnectionIdentifier): Box[Connection] = {
    ...
    trace("got conn: %s (%s/%s)".format(c, DataSource.getNumActive(), DataSource.getNumIdle()))
    c
  }
  override def releaseConnection(conn: Connection): Unit = {
    conn.close()
    trace("released conn: %s (%s/%s)".format(conn, DataSource.getNumActive(), DataSource.getNumIdle()))
  }
...
class SqlHistoryListener(override val name:String, driver:String, url:String, username:Option[String], password:Option[String], settings:DBSettings = DBSettings()) extends PushingToRemoteHistoryListener(name) with Safely {
  object LocalConnectionIdentifier extends NamedConnectionIdentifier("%s_%s_%s".format(driver,url,username))
  lazy val vendor = DBVendors.getVendor(driver, url, username, password, settings)

  object DBCheckResult extends DBCheckResult with LongKeyedMetaMapper[DBCheckResult] {
    ...
    def fromCheckResult(in:CheckResult):DBCheckResult = {
      ...
      val data = JObject(List(
        ...
        JField("data",JArray(in.data.map(d => {
          JObject(List(
            JField("when",JInt(d._1)),
            JField("values",JObject(d._2.toList.map(dTup => {
    ...
    def toCheckResult(in:DBCheckResult):CheckResult = {
      ...
      CheckResult(in.crId.get,serviceCheck,label,in.service.get,serviceLabel,in.server.get,serverLabel,new Date(in.when.get),why,tryo(new Date(in.lastUp.get)),detail,ServiceCheckMode.parse(in.mode.get),ServiceCheckSeverity.parse(in.severity.get),in.success.get,parsedData,tryo(in.duration.get))
    }
  }
  ...
  Schemifier.schemify(true,Schemifier.infoF _,LocalConnectionIdentifier,DBCheckResult)
  ...
  override def getHistoryFor(service:String, server:String, serviceCheck:String, after:Option[Long], limit:Option[Int]):List[CheckResult] = { 
    val terms:List[QueryParam[DBCheckResult]] = List(By(DBCheckResult.service,service),By(DBCheckResult.server,server),By(DBCheckResult.serviceCheck,serviceCheck)) ::: 
      after.toList.map(a => Cmp(DBCheckResult.when,OprEnum.>,Full(a),Empty,Empty)) ::: 
      limit.toList.map(l => new MaxRows[DBCheckResult](l.toLong))
    withDb(DBCheckResult.findAll(terms:_*).map(DBCheckResult.toCheckResult _)).right.toOption.getOrElse(Nil)
  }
  override def getAllHistory(after:Option[Long],limit:Option[Int]):List[List[CheckResult]] = {
    val terms:List[QueryParam[DBCheckResult]] = after.toList.map(a => Cmp(DBCheckResult.when,OprEnum.>,Full(a),Empty,Empty)) ::: limit.toList.map(l => new MaxRows[DBCheckResult](l.toLong))
    withDb(List(DBCheckResult.findAll(terms:_*).map(DBCheckResult.toCheckResult _))).right.toOption.getOrElse(Nil)
  }
	override def performRepeatableAtomicAction(cr:CheckResult):Boolean = {
    withDb(DBCheckResult.fromCheckResult(cr).save).right.toOption.getOrElse(false)
	}
}
