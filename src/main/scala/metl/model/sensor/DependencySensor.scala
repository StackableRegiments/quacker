...
case class DependencySensor(
  ...
  failureTolerance = 3
  def status = {
    val (successes, errors) = dependencies.toList.foldLeft(
      (List.empty[(DependencyDescription, DependencyMatcher)],
       List.empty[String]))((acc, item) => {
      val desc = item._1
      val matcher = item._2
    ...
    if (errors.length > 0) {
      throw new DashboardException("DependencyCheck Verification failed",
                                   errors.mkString("\r\n"))
    } else {
      successes
        .map(s => {
          "(%s_%s_%s) => %s".format(s._1.service.getOrElse(""),
                                    s._1.server.getOrElse(""),
                                    s._1.pinger,
                                    s._2.describe)
        })
        .mkString(" AND ")
