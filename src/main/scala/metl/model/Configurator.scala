...
object ServiceConfigurator extends Logger {
		...
		path.listFiles.sortWith(_.getName < _.getName).map(f => {
			...
			if (f.isDirectory){
				if (f.getName == ".git" || f.getName == ".svn" || f.getName == ".hg" || f.getName == "_svn" || f.getName == "_git" || f.getName == "_hg"){
					items = items.updated(pathName,(false,"ignoring version-control data directory: %s".format(pathName)))
				} else if (f.getName.endsWith(".disabled")){
					items = items.updated(pathName,(false,"skipping directory: %s".format(pathName)))
				} else {
	...
	def loadServicesFromPath(xmlPath:String):Tuple2[Boolean,String] = {
		try {
			...
			case e:Throwable => {
				(false,"failed to load %s with exception: %s -> %s".format(xmlPath,e,e.getMessage))
			}
		}
	}
}
